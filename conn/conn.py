...
def loadData():
    ...
    bins['layerLabels'] = ['pia', 'L1-L2 border', 'L2/3-L4 border', 'L4-L5A border', 'L5A-L5B border', 'L5B-L6 border',
                           'L6-WM border']
    ...
    binSize = 0.10
    bins['W'] = np.array(
        [np.arange(binSize, 1.0, binSize), np.arange(2 * binSize, 1.0 + binSize, binSize)]).T  # bin edges
...
def interp2mat(ngridIN, ptsIN, ngridOUT, binsOUT, fillval=0.0):
    ...
    matOUT = np.zeros((len(binsOUT), len(binsOUT)));
    idx = 0
...
def plotMatsEE():
    ...
    labelPostBins = [('W+AS', 'IT', 'L2/3,4'), ('W+AS', 'IT', 'L5A,5B'), ('W+AS', 'PT', 'L5B'), ('W+AS', 'IT', 'L6'),
                     ('W+AS', 'CT', 'L6')]
    ...
    for i, (label, preBin, postBin) in enumerate(zip(labels, labelPreBins, labelPostBins)):
        ...
        im_smat = plt.imshow(smat[label], origin='lower', interpolation='None', aspect='auto', extent=xylims, vmin=0,
                             vmax=maxVal)
    ...
    for i, (label, preBin, postBin) in enumerate(zip(labels, labelPreBins, labelPostBins)):
        plt.subplot(len(labels), 1, i + 1)
        xylims = (bins[preBin][0][0], bins[preBin][-1][-1], bins[postBin][0][0], bins[postBin][-1][-1])
        im_pmat = plt.imshow(pmat[label], origin='lower', interpolation='None', aspect='auto', extent=xylims, vmin=0,
                             vmax=maxVal)
        # title('E -> '+label[1]+' '+label[2])
        if i != 4:
            plt.setp(plt.gca().get_xticklabels(), visible=False)
    ...
    for i, (label, preBin, postBin) in enumerate(zip(labels, labelPreBins, labelPostBins)):
        plt.subplot(len(labels), 1, i + 1)
        xylims = (bins[preBin][0][0], bins[preBin][-1][-1], bins[postBin][0][0], bins[postBin][-1][-1])
        im_wmat = plt.imshow(wmat[label], origin='lower', interpolation='None', aspect='auto', extent=xylims, vmin=0,
                             vmax=maxVal)
        # title('E -> '+label[1]+' '+label[2])
        # if i == 4:
        #     xlabel(' . ', fontsize=18);
...
def plotMatsEEHoriz():
    ...
    mpl.rc('font', **{'family': 'sans-serif', 'sans-serif': ['Arial']})
    mpl.rcParams.update({'font.size': 12})
    labels = [('W+AS_norm', 'IT', 'L2/3,4'), ('W+AS_norm', 'IT', 'L5A,5B'), ('W+AS_norm', 'IT', 'L6'),
              ('W+AS_norm', 'PT', 'L5B'), ('W+AS_norm', 'CT', 'L6')]
    labelPostBins = [('W+AS', 'IT', 'L2/3,4'), ('W+AS', 'IT', 'L5A,5B'), ('W+AS', 'IT', 'L6'), ('W+AS', 'PT', 'L5B'),
                     ('W+AS', 'CT', 'L6')]
    ...
    for i, (label, preBin, postBin) in enumerate(zip(labels, labelPreBins, labelPostBins)):
        ...
        im_smat = plt.imshow(smatLabel, origin='lower', interpolation='None', aspect='auto', extent=xylims, vmin=0,
                             vmax=maxVal)
...
def plotMatsEI():
    ...
    for i, label in enumerate(labels):
        ...
        im = plt.imshow(rescale(smat[label], bins['FS/LTS'], 0.01), origin='lower', interpolation='None', aspect='auto',
                        extent=xylims, vmin=0, vmax=maxVal)
        # title('E -> '+label)
        if i != 1:
            plt.setp(plt.gca().get_xticklabels(), visible=False)
        # if i == 1:
    ...
    for i, label in enumerate(labels):
        plt.subplot(len(labels), 1, i + 1)
        xylims = (bins['FS/LTS'][0][0], bins['FS/LTS'][-1][-1], bins['FS/LTS'][0][0], bins['FS/LTS'][-1][-1])
        im = plt.imshow(rescale(pmat[label], bins['FS/LTS'], 0.01), origin='lower', interpolation='None', aspect='auto',
                        extent=xylims, vmin=0, vmax=maxVal)
        # title('E -> '+label)
        if i != 1:
            plt.setp(plt.gca().get_xticklabels(), visible=False)
    ...
    for i, label in enumerate(labels):
        plt.subplot(len(labels), 1, i + 1)
        xylims = (bins['FS/LTS'][0][0], bins['FS/LTS'][-1][-1], bins['FS/LTS'][0][0], bins['FS/LTS'][-1][-1])
        im = plt.imshow(rescale(wmat[label], bins['FS/LTS'], 0.01), origin='lower', interpolation='None', aspect='auto',
                        extent=xylims, vmin=0, vmax=maxVal)
        # title('E -> '+label)
        # if i == 1:
        #      xlabel(' . ', fontsize=18);
...
for celltype in ['LTS', 'FS']:
    ...
    pmat[celltype][2, 2] = pmat[('W+AS_norm', 'IT', 'L6')][:,
                           W_6_bins].mean() * EtoIprobFactor * 1.5  # increase L6 CT/IT->I (Yamawaki & Shepeherd 2015)
...
pmat['LTS'][1, 0] = 1.5 * pmat[
    'LTS'].max()  # pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,2:4].max() # increase IT L2/3 -> LTS L5A/B prob
pmat['FS'][1, 0] = 0.5 * pmat[
    'FS'].min()  # pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,2:4].min() # decrease IT L2/3 -> FS L5A/B prob
...
pmat['FS'][1, 1] = 1.5 * pmat[
    'FS'].max()  # pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,2:11].max() # increase IT/PT L5A/B -> FS L5A/B prob (use L2/3 values)
pmat['LTS'][1, 1] = 0.5 * pmat[
    'LTS'].min()  # pmat[('W+AS_norm', 'IT', 'L5A,5B')][:,5:6].min() # decrease IT/PT L5A/B -> LTS L5A/B prob (>0)
