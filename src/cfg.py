...
cfg.cellsrec = 0
if cfg.cellsrec == 0:
    cfg.recordCells = ['PT5B']  # all record all cells
elif cfg.cellsrec == 1:
...
# Saving
# ------------------------------------------------------------------------------
cfg.simLabel = 'v103_tune_%s_treatment_%s' % (
cfg.variant, str(cfg.treatment)) if cfg.loadmutantParams else 'v103_tune_WT_treatment%s' % str(cfg.treatment)
...
cfg.analysis['plotRaster'] = {'include': allpops, 'orderBy': ['pop', 'y'], 'timeRange': [0, cfg.duration],
                              ...
                              'popColors': popColors, 'figSize': (18, 18), 'lw': 0.3, 'markerSize': 3, 'marker': '.',
                              'dpi': 100}
